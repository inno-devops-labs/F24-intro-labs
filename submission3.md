...
'git reset --soft': This command moves the HEAD pointer to an earlier commit but keeps changes in the staging area. This allows to re-commit those changes if needed.
...
'git reflog': This command shows a log of all HEAD movements, allowing to recover commits that were lost after a reset or other operations that modify commit history.
